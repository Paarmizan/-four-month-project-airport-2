...
let sendButton = document.querySelector('.form__button-submit');

class incorrectTicketValue {
	constructor(message) {
		this.message = message;
		this.name = "incorrectTicketValue";
	}
}

sendButton.onclick = function() {
	try {
		if(ticket.value.length != 10) {
			ticket.value = '';
			throw new incorrectTicketValue("Введите 10-ти значный код билета!");
		}
	} catch (error) {
		alert(error.message);
	}
